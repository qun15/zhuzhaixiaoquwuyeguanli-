...
<script type="text/javascript">
	...
	function showRemarkList(){
      $.ajax({
		  url:"workbench/activity/getRemakeListByAid.do",
		  data:{
			  activityId:"${a.id}"
		  },
		  type:"get",
	...
	function deleteRemark(id) {
		$.ajax({
			...
			success: function (data) {
				if (data.success) {
					//showRemarkList()不可以,每一次删除都会保留原有记录
					$("#"+id).remove();
				} else {
					alert("删除备注失败")
				}
...
<body>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		<div class="page-header">
			<h3>住户投诉-${a.name} <small>${a.startDate} ~ ${a.endDate}</small></h3>
		</div>
	...
	<div style="position: relative; top: -70px;">
		<div style="position: relative; left: 40px; height: 30px;">
			<div style="width: 300px; color: gray;">投诉者</div>
